...
async function sendVerificationEmail(toEmail, userName, code) {
    const htmlContent = `
        <div style="direction:rtl; font-family: Vazir, sans-serif; max-width: 500px; margin: 0 auto; padding: 30px 20px; background: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0;">
            <h2 style="text-align:center; color:#FF6B00;">AYS</h2>
            <p style="font-size:16px; color:#1e293b;">سلام ${userName}،</p>
            <p style="font-size:16px; color:#1e293b;">کد تأیید شما برای ثبت‌نام در <strong>AYS</strong>:</p>
            <div style="background: white; padding: 20px; border-radius: 12px; text-align: center; margin: 20px 0; border: 2px dashed #FF6B00;">
                <span style="font-size: 32px; font-weight: 900; color: #FF6B00; letter-spacing: 8px;">${code}</span>
            </div>
            ...
            <p style="font-size:12px; color:#94a3b8; text-align:center;">AYS | جایی که ایده‌های شما خریداری می‌شود</p>
        </div>
    `;
    ...
    await transporter.sendMail({
        ...
        to: toEmail,
        subject: 'کد تأیید ثبت‌نام در AYS',
        html: htmlContent
    });
}
